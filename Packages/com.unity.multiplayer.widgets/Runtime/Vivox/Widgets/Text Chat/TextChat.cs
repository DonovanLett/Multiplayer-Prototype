    ...
    internal class TextChat : WidgetBehaviour, IChatEvents, ISessionProvider, ISessionLifecycleEvents
    {
        ...
        void Start()
        {
            ...
            if (ChatManager.Instance != null &&
                ChatManager.Instance.IsChatJoined &&
                ChatManager.Instance.CurrentChatId == Session.Id)
            {
                OnChatJoined(Session.Id);
            }
            */
        }
        // Added Chat Code
        ...
        // Added Chat Code
        void SynchronizeChatState()
        {
            if (ChatManager.Instance == null)
                return;

            if (!ChatManager.Instance.IsChatJoined)
                return;

            if (Session == null)
                return;

            if (ChatManager.Instance.CurrentChatId != Session.Id)
                return;
        ...
        {
            Debug.Log("========== TEXT CHAT SERVICES INITIALIZED =========="); // Added Code
        ...
        {
            Debug.Log($"========== CHAT JOINED: {chatId} ==========");

            m_InputField.interactable = true;
            Debug.Log("Chat Entered Successfully!!");
        }
